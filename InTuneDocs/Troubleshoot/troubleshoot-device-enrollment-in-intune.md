...
title: Problemen bij de apparaatinschrijving oplossen | Microsoft Intune
description:
keywords:
author: Nbigman
manager: jeffgilb
...
Hier worden een aantal problemen vermeld die zich kunnen voordoen bij het inschrijven van apparaten en wordt beschreven hoe u deze kunt oplossen.

> [!NOTE]
> Gebruikers van door u beheerde apparaten kunnen inschrijvings- en diagnostische gegevens laten vastleggen in logboeken, zodat u deze kunt bekijken. Gebruikersinstructies voor het vastleggen van gegevens in logboeken vindt u in:
...
>- [iOS-inschrijvingsfouten verzenden naar de IT-beheerder](/intune/enduser/send-errors-to-your-it-admin-ios)
...
**Probleem:** een gebruiker ontvangt tijdens het inschrijven een foutbericht op het apparaat, zoals het foutbericht **Bedrijfsportal is tijdelijk niet beschikbaar** op een iOS-apparaat, en DMPdownloader.log in Configuration Manager bevat de fout **DeviceCapReached**..
...
1.  Blader naar [http://aka.ms/accessaad](http://aka.ms/accessaad) of klik op **Beheer** &gt; **Azure AD** op [https://portal.office.com](https://portal.office.com)..

2.  Meld u aan met uw organisatie-id via de koppeling aan de linkerkant van de pagina.

3.  Een Azure-abonnement maken als u er nog geen hebt. U kunt dit zonder creditcard of betaling doen als u beschikt over een betaald account (klik op de abonnementskoppeling **Uw gratis Azure Active Directory registreren** ).
...
**Probleem:** een gebruiker ontvangt op zijn apparaat de fout **Bedrijfsportal tijdelijk niet beschikbaar**.
...
**Probleem:** een gebruiker ontvangt de fout **MDM-instantie is niet gedefinieerd**.
...
3.  Als u gebruikmaakt van System Center Configuration Manager met Intune, controleert u of de gebruiker over een geldige cloudgebruikers-id beschikt:
    ...
    4.  Klik bovenaan op Nieuwe query en voer de volgende query's uit:

        -   Als u alle gebruikers wilt weergeven:
            `select * from [CM_ DBName].[dbo].[User_DISC]`

        -   Als u specifieke gebruikers wilt weergeven, gebruikt u de volgende query waarbij %testuser1% staat voor username@domain.com voor de gebruiker die u wilt weergeven:
            `select * from [CM_ DBName].[dbo].[User_DISC] where User_Principal_Name0 like '%testuser1%'`

        Nadat u de query hebt opgesteld, klikt u op **!Uitvoeren**..
...
**Oplossing:** dit probleem kan optreden omdat er een aangepast proces actief is dat apparaten verwijdert die niet aan een domein zijn toegevoegd of omdat de gebruiker het apparaat uit het abonnement heeft verwijderd. Als u wilt nagaan door welk gebruikersaccount of proces het apparaat uit de Configuration Manager-console is verwijderd, moet u de volgende stappen uitvoeren.
...
4.  Zoek het betreffende apparaat en bekijk hoe het is verwijderd. In het volgende voorbeeld ziet u dat het apparaat via een onbekende toepassing is verwijderd door het account SCCMInstall.
    ...
    ![Schermopname voor diagnose apparaatverwijdering](./media/CM_With_Intune_Unknown_App_Deleted_Device.jpg)
...
**Probleem:** een gebruiker schrijft zijn iOS-apparaat in, maar het apparaat wordt niet weergegeven in de beheerconsole van Configuration Manager. Op het apparaat wordt niet aangegeven dat het is ingeschreven. Mogelijke oorzaken:

- Misschien hebt u uw Intune Connector ingeschreven bij het ene account en vervolgens bij een ander account. 
...
1. Schakel iOS uit in de Windows Intune Connector. 
    1. Klik met de rechtermuisknop op het Intune-abonnement en kies Eigenschappen.
    1. Schakel op het tabblad iOS de optie iOS-inschrijving inschakelen uit.
...
1. Start de service SMS Executive opnieuw op of start de CM-server opnieuw op 



1. Haal een nieuw APN-certificaat op en upload dit: klik met de rechtermuisknop op het Intune-abonnement in het linkerdeelvenster van Configuration Manager. Kies **APNs-certificaataanvraag maken** en volg de instructies.


## De computer is al ingeschreven: fout hr 0x8007064c
**Probleem:** inschrijving mislukt met de fout **De computer is al ingeschreven**. In het inschrijvingslogboek wordt fout **hr 0x8007064c** vermeld.
  
Dit komt mogelijk doordat de computer eerder is ingeschreven of een gekloonde kopie bevat van een computer die is ingeschreven. Het accountcertificaat van het vorige account staat nog op de computer.
...
1. Zoek naar het Intune-certificaat dat is uitgegeven door Sc_Online_Issuing en verwijder dit als het aanwezig is
1. Verwijder de volgende registersleutel en alle subsleutels als deze bestaat: ** HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\OnlineManagement regkey**.
1. Probeer de computer opnieuw in te schrijven. 
...
1. Probeer de computer opnieuw in te schrijven. 
    ...
    > [!IMPORTANT]
    > Deze sectie, methode of taak bevat stappen voor het wijzigen van het register. Als u het register onjuist bewerkt, kunnen er echter ernstige problemen optreden. Zorg er daarom voor dat u deze stappen zorgvuldig uitvoert. Maak voor de zekerheid een back-up van het register voordat u het aanpast. Vervolgens kunt u het register herstellen als er een probleem optreedt.
    > Lees [Back-ups van het register maken en herstellen in Windows](https://support.microsoft.com/en-us/kb/322756) voor meer informatie over het maken en herstellen van een back-up van het register

## Kan geen beleid maken of apparaten inschrijven als de bedrijfsnaam speciale tekens bevat
**Probleem:** u kunt geen beleid maken of apparaten inschrijven.
...
## U kunt zich niet aanmelden of apparaten inschrijven wanneer u meerdere geverifieerde domeinen hebt
**Probleem:** als u een tweede geverifieerd domein toevoegt aan uw AD FS, kunnen gebruikers met het UPN-achtervoegsel (user principal name) van het tweede domein zich mogelijk niet aanmelden bij de portals of kunnen ze geen apparaten inschrijven. 


**Oplossing:** Microsoft Office 365-klanten die gebruikmaken van eenmalige aanmelding (SSO) via AD FS 2.0 en meerdere domeinen op het hoogste niveau hebben voor UPN-achtervoegsels van gebruikers in hun organisatie (bijvoorbeeld @contoso.com of @fabrikam.com), moeten voor elk achtervoegsel een afzonderlijk exemplaar van de AD FS 2.0 Federation Service implementeren.  Er is nu een [rollup voor AD FS 2.0](http://support.microsoft.com/kb/2607496) die kan worden gebruikt met de schakeloptie **SupportMultipleDomain** om de AD FS-server in te schakelen voor ondersteuning van dit scenario zonder extra AD FS 2.0-servers. Lees [deze blog](https://blogs.technet.microsoft.com/abizerh/2013/02/05/supportmultipledomain-switch-when-managing-sso-to-office-365/) voor meer informatie.
...
|0x80043006, 0x80CF3006|Het maximumaantal toegestane seats voor het account is bereikt.|Uw organisatie moet extra plaatsen aanschaffen voordat u meer clientcomputers in de service kunt inschrijven.|
...
|0x80043008, 0x80CF3008|Het starten van de Microsoft Online Management Updates-service is mislukt.|Neem contact op met Microsoft Ondersteuning, zoals wordt beschreven in [Ondersteuning voor Microsoft Intune krijgen](how-to-get-support-for-microsoft-intune.md).|
...
|0x80cf0440|De verbinding met het service-eindpunt is beëindigd.|Proef- of betaald account is onderbroken. Maak een nieuw proef- of betaald account en schrijf u opnieuw in.|


## iOS-inschrijvingsfouten
In de gebruikersdocumentatie van het apparaat vindt u in [Er worden fouten weergegeven tijdens het inschrijven van het iOS-apparaat in Intune](/intune/enduser/using-your-ios-or-mac-os-x-device-with-intune) een lijst met andere iOS-inschrijvingsfouten.
