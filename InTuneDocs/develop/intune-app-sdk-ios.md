...
description: Met de Microsoft Intune App SDK voor iOS kunt u beveiligingsbeleid voor Intune-apps, in de vorm van Mobile App Management (MAM), opnemen in uw iOS-app.
keywords: 
author: mtillman
manager: angrobe
ms.author: mtillman
...
> [!NOTE]
> U kunt desgewenst eerst het artikel [Aan de slag met Intune App SDK](intune-app-sdk-get-started.md) lezen dat ingaat op de huidige functies van de SDK en waarin wordt beschreven hoe u de integratie voor elk ondersteund platform kunt voorbereiden.
...
* Download de bestanden voor de Intune App SDK voor iOS vanaf [GitHub](https://github.com/msintuneappsdk/ms-intune-app-sdk-ios).

## <a name="whats-in-the-sdk"></a>Inhoud van de SDK

De Intune App SDK voor iOS bevat een statische bibliotheek, resourcebestanden, API-headers, een PLIST-bestand met foutopsporingsinstellingen en een configuratiehulpprogramma. Makers van mobiele apps kunnen voor de meeste beleidsafdwinging eenvoudigweg de resourcebestanden opnemen en statische koppelingen maken naar de bibliotheken. Geavanceerde MAM-functies van Intune worden afgedwongen met behulp van API's.
...
* **Headers**: beschrijft de Intune App SDK-API’s. Als u een API gebruikt, moet u het headerbestand met de API opnemen. De volgende headerbestanden bevatten de API-functieaanroepen die zijn vereist om de functionaliteit van de Intune App SDK in te schakelen:
...
## <a name="build-the-sdk-into-your-mobile-app"></a>De SDK inbouwen in uw mobiele app
...
2. **Optie 2**: maak een koppeling naar de `libIntuneMAM.a`-bibliotheek. Sleep de `libIntuneMAM.a`-bibliotheek naar de lijst **Linked Frameworks and Libraries** van het projectdoel.

    ![Intune App SDK iOS: gekoppelde frameworks en bibliotheken](../media/intune-app-sdk-ios-linked-frameworks-and-libraries.png)

    > [!NOTE]
    > Als u van plan bent om uw app vrij te geven in de App Store, gebruik dan de releaseversie van `libIntuneMAM.a` en niet de foutopsporingsversie. De releaseversie bevindt zich in de map **Release**. De foutopsporingsversie biedt uitgebreide uitvoer die helpt bij het oplossen van problemen met de Intune App SDK.
    ...
    Voeg `-force_load {PATH_TO_LIB}/libIntuneMAM.a` toe aan van de volgende items, waarbij u `{PATH_TO_LIB}` vervangt door de locatie van de Intune App SDK:
      * De `OTHER_LDFLAGS`-buildconfiguratie-instelling van het project
      * De **Other Linker Flags** van de gebruikersinterface

        > [!NOTE]
        > Om te zoeken naar `PATH_TO_LIB`, selecteert u het bestand `libIntuneMAM.a` en kiest u **Get Info** in het menu **File**. Kopieer en plak de informatie onder **Where** (het pad) vanuit het gedeelte **General** van het venster **Info**.
...
4. Voeg de resourcebundel `IntuneMAMResources.bundle` toe aan het project door deze binnen **Build Phases** naar **Copy Bundle Resources** te slepen.
    ...
    ![Intune App SDK iOS: resourcebundels kopiëren](../media/intune-app-sdk-ios-copy-bundle-resources.png)

5. Als uw mobiele app een hoofd-nib of hoofd-storyboard definieert in het bestand info.plist, verwijdert u het veld **Main Storyboard** en/of **Main Nib**. In Info.plist plakt u deze velden en de bijbehorende waarden in een nieuwe woordenlijst met de naam **IntuneMAMSettings** met de volgende sleutelnamen, voor zover van toepassing:
    * MainStoryboardFile
  ...
  > Als uw mobiele app geen hoofd-nib of -storyboard in het bestand Info.plist definieert, zijn deze instellingen niet vereist.
    ...
    U kunt info.plist in een onbewerkte indeling weergeven (om de sleutelnamen te kunnen bekijken) door met de rechtermuisknop op een willekeurige plaats in de hoofdtekst van het document te klikken en het weergavetype te wijzigen in **Show Raw Keys/Values**.

6. Schakel het delen van sleutelketens in (indien nog niet ingeschakeld) door in elk projectdoel op **Capabilities** te klikken en de schakelaar **Keychain Sharing** in te schakelen. Het delen van sleutelketens is vereist als u wilt doorgaan met de volgende stap.

  > [!NOTE]
    > Uw inrichtingsprofiel moet ondersteuning bieden voor nieuwe waarden voor het delen van sleutelketens. De toegangsgroepen van de sleutelketen moeten een jokerteken ondersteunen. U kunt dit controleren door het bestand .mobileprovision in een teksteditor te openen, te zoeken naar **keychain-access-groups** en te controleren of u een jokerteken ziet. Bijvoorbeeld:
    ```xml
...
7. Nadat u het delen van sleutelketens hebt ingeschakeld, volgt u deze stappen voor het maken van een afzonderlijke toegangsgroep waarin de gegevens van de Intune App SDK worden opgeslagen. U kunt een toegangsgroep voor de sleutelketen maken via de gebruikersinterface of met behulp van het rechtenbestand. Als u de toegangsgroep voor de sleutelketen maakt met de gebruikersinterface, moet u de onderstaande stappen uitvoeren:
    ...
    3. Voeg `com.microsoft.adalcache` aan uw bestaande toegangsgroepen toe.
        ...
        4. Voeg `com.microsoft.workplacejoin` aan uw bestaande toegangsgroepen toe.
            ![Intune App SDK iOS: sleutelketens delen](../media/intune-app-sdk-ios-keychain-sharing.png)
...
7. Als er voor de app URL-schema's in het info.plist-bestand zijn gedefinieerd, voegt u voor elk URL-schema een extra schema toe, met het achtervoegsel `-intunemam`.
...
2. Neem de resourcebundel `ADALiOSBundle.bundle` op in het project door deze binnen **Build Phases** naar **Copy Bundle Resources** te slepen.
...
2. Schakel eenmalige aanmelding van ADAL in door toegangsgroepen `com.microsoft.adalcache` en `com.microsoft.workplacejoin` toe te voegen aan de sleutelketenrechten.
...
Als uw app al ADAL gebruikt, zijn de volgende configuraties vereist:
...
```
Door de methode `registerAndEnrollAccount` aan te roepen, kan het gebruikersaccount door de SDK worden geregistreerd en kan de app namens dit account worden ingeschreven. Als de inschrijving om welke reden dan ook mislukt, onderneemt de SDK 24 uur later automatisch een nieuwe poging om de app in te schrijven. Om fouten te kunnen opsporen, kan de app via een gemachtigde meldingen ontvangen over de resultaten van een inschrijvingsaanvraag.
...
Voordat de gebruiker wordt afgemeld, moet de app de volgende API aanroepen in `Headers/IntuneMAMEnrollment.h`:
...
### <a name="apps-that-do-not-use-adal"></a>Apps die geen gebruikmaken van ADAL
...
## <a name="customize-your-apps-behavior"></a>Het gedrag van uw app aanpassen

De Intune App SDK heeft verschillende API's die u kunt aanroepen met informatie over het Intune-beveiligingsbeleid voor apps dat op de app is geïmplementeerd. U kunt met deze gegevens het gedrag van uw app aanpassen. De meeste instellingen van het beveiligingsbeleid voor apps worden automatisch afgedwongen door de SDK en niet de toepassing. De enige instelling die de app moet implementeren is het besturingselement Opslaan als.
...
U kunt de **IntuneMAMSettings**-woordenlijst gebruiken die deel uitmaakt van het bestand Info.plist van de toepassing voor het configureren en instellen van de Intune App SDK. Als de woordenlijst IntuneMAMSettings niet in het bestand Info.plist wordt vermeld, moet u een woordenlijst maken in Info.plist van uw app, met de veldnaam IntuneMAMSettings.

U kunt in de woordenlijst IntuneMAMSettings verschillende rijen met sleutels en waarden van configuratie-instellingen toevoegen om de SDK te configureren. De volgende tabel bevat alle ondersteunde instellingen.
...
ADALLogOverrideDisabled | Boolean-waarde  | Geeft aan of de SDK alle ADAL-logboeken (inclusief ADAL-aanroepen van de app, indien van toepassing) naar een eigen logboekbestand routeert. De standaardwaarde is NO (Nee). De waarde is YES (Ja) als de app een eigen ADAL-logboek aanroept. | Optioneel. |
ADALCacheKeychainGroupOverride | Tekenreeks  | Hiermee wordt aangegeven welke sleutelketengroep voor de ADAL-cache moet worden gebruikt in plaats van 'com.microsoft.adalcache'. Houd er rekening mee dat deze niet het voorvoegsel app-id heeft. Dat wordt tijdens runtime als voorvoegsel toegevoegd aan de gegeven tekenreeks. | Optioneel. |
...
MainNibFile<br>MainNibFile~ipad  | Tekenreeks  | Deze instelling moet de bestandsnaam van de hoofd-nib van de app hebben.  | Vereist als de app MainNibFile definieert in Info.plist. |
...
MAMPolicyRequired| Boolean-waarde| Deze instelling geeft aan of de app is geblokkeerd en niet kan worden gestart als de app geen Intune-beveiligingsbeleid voor apps heeft. De standaardwaarde is NO (Nee). <br><br> Opmerking: apps kunnen niet worden verzonden naar de APP Store als de MAMPolicyRequired is ingesteld op Yes (Ja). | Optioneel. |
MAMPolicyWarnAbsent | Boolean-waarde| Deze instelling geeft aan of de app tijdens het opstarten een waarschuwing verzendt naar de gebruiker als de app geen Intune-beveiligingsbeleid voor apps heeft. Apps kunnen niet worden verzonden naar de Store als deze instelling is ingesteld op YES. | Optioneel. |
...
AccentColor | Tekenreeks| Hiermee geeft u de accentkleur aan voor schermen waarop een pincode moet worden ingevoerd, voor bijvoorbeeld knoptekst en de kleur voor het markeren van vakken. Accepteert een hexadecimale RGB-tekenreeks in de vorm #XXXXXX, waarbij elke X een teken van 0-9 of A-F kan zijn. Het hekje kan worden weggelaten.| Optioneel. De standaardkleur is systeemblauw. |
...
> [!NOTE]
> Als uw app wordt gepubliceerd naar de App Store, moet `MAMPolicyRequired` worden ingesteld op NO (Nee), volgens de App Store-standaarden.
...
* **Gebruikersinterface-identiteit**: bepaalt welk beleid wordt toegepast op gebruikersinterfacetaken in de hoofdthread, zoals knippen/kopiëren/plakken, pincode, verificatie en gegevens delen. De gebruikersinterface-identiteit heeft geen invloed op taken zoals versleuteling en het maken van back-ups.
...
Elke thread heeft op elk moment een effectieve identiteit voor gebruikersinterface- en bestandstaken. Dit is de identiteit die wordt gebruikt om te controleren welk beleid er eventueel moet worden toegepast. Als de identiteit 'geen identiteit' is of als de gebruiker niet wordt beheerd, wordt er geen beleid toegepast. De volgende diagrammen tonen hoe de effectieve identiteiten worden bepaald.

  ![Intune App SDK iOS: gekoppelde frameworks en bibliotheken](../media/intune-app-sdk/ios-thread-identities.png)

### <a name="thread-queues"></a>Threadwachtrijen

Apps verzenden vaak asynchrone en synchrone taken naar threadwachtrijen. De SDK onderschept GCD-aanroepen (Grand Central Dispatch) en koppelt de huidige threadidentiteit aan de verzonden taken. Wanneer de taken zijn voltooid, wijzigt de SDK tijdelijk de threadidentiteit in de identiteit die is gekoppeld aan de taak, waarna de SDK de taken voltooit en de oorspronkelijke threadidentiteit teruggezet.


Omdat `NSOperationQueue` boven op GCD is gebouwd, worden `NSOperations` uitgevoerd op de identiteit van de thread op het tijdstip dat de taken worden toegevoegd aan de `NSOperationQueue`. `NSOperations` of functies die rechtstreeks worden verzonden via GCD hebben ook de mogelijkheid om de huidige threadidentiteit te wijzigen terwijl ze worden uitgevoerd. Deze identiteit overschrijft de identiteit die is overgenomen van de verzendende thread.
...
De methode `protect` accepteert een identiteit die een beheerde of een niet-beheerde gebruiker kan zijn. Als het een beheerde gebruiker betreft, worden de gegevens versleuteld. Als het een niet-beheerde gebruiker betreft, wordt een header toegevoegd aan de gegevens waarmee de identiteit wordt gecodeerd, maar worden de gegevens niet versleuteld. U kunt de methode `protectionInfo` gebruiken voor het ophalen van de eigenaar van de gegevens.
...
* **Door SDK geïnitieerde overschakeling naar andere identiteit**:
    ...
    Wanneer deze methode wordt aangeroepen, en als de app de aanvraag kan afhandelen om naar een opgegeven identiteit over te schakelen, moet de app `IntuneMAMAddIdentityResultSuccess` doorgeven aan de voltooiingshandler. Als de app de aanvraag om naar een andere identiteit over te schakelen niet kan afhandelen, moet de app `IntuneMAMAddIdentityResultFailed` doorgeven aan de voltooiingshandler.
...
* **Selectief wissen**:
    ...
    Wanneer de app selectief wordt gewist, zal de SDK de `wipeDataForAccount`-methode in `IntuneMAMPolicyDelegate` aanroepen. Het is de verantwoordelijkheid van de app om het opgegeven gebruikersaccount en alle bijbehorende gegevens te verwijderen. De SDK is in staat om alle bestanden te verwijderen waarvan de gebruiker de eigenaar is, en zal dit doen als de app FALSE (onwaar) retourneert als reactie op de `wipeDataForAccount`-aanroep.
...
Nee. In feite moeten alleen werk- of schoolaccounts worden geregistreerd bij de Intune App SDK. Apps zijn er verantwoordelijk voor om te bepalen of een account wordt gebruikt in de context van werk of school.   
...
1. Zorg ervoor dat `IntuneMAM.framework` zich op uw bureaublad bevindt.
