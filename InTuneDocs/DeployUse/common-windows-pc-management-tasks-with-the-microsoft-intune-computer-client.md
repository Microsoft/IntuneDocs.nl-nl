...
Bestudeer de taken in dit onderwerp voor meer informatie over het beheren van computers met de Intune-client. Zie [De Windows-pc-client installeren met Microsoft Intune](install-the-windows-pc-client-with-microsoft-intune.md) als u de client nog niet op uw computers hebt geïnstalleerd.
...
-   Afhankelijk van de configuratie kunnen computers die buiten gebruik zijn gesteld, nog steeds updates ontvangen via Windows Server Update Services, Windows Update of Microsoft Update.
    ...
    > [!IMPORTANT] Als de clientsoftware is geïnstalleerd met behulp van een groepsbeleidsobject (GPO), moet u het groepsbeleidsobject verwijderen voordat u de clientsoftware kunt verwijderen om te voorkomen dat de software wordt geïnstalleerd.
...
> [!TIP] Als u de mogelijkheden van eindgebruikers om zichzelf aan computers te koppelen wilt beperken, schakelt u de optie **Koppelingen tussen gebruikers en computers beperken** in het beleid voor **Instellingen Microsoft Intune-agent** in.

## Hulp op afstand aanvragen en verlenen aan Windows-pc's die gebruikmaken van de Intune-clientsoftware

> [!IMPORTANT]
> Het is mogelijk dat de configuratieopties voor de integratie van TeamViewer niet worden weergegeven in de Intune-beheerconsole. Deze mogelijkheid is momenteel niet beschikbaar voor alle klanten, maar zal binnenkort breder worden geïmplementeerd.
...
Voordat u aanvragen voor hulp op afstand kunt uitvoeren en beantwoorde, moet aan de volgende vereisten zijn voldaan
