...
- **Het e-mailprofiel selecteren dat moet worden beheerd door Intune**: Als de instelling **E-mailaccount moet worden beheerd door Intune** is geselecteerd, kiest u **Selecteren** om het e-mailprofiel voor Intune op te geven. Het e-mailprofiel moet aanwezig zijn op het apparaat.
...
- **Opengebroken apparaten**: Als u deze instelling inschakelt, geeft u aan dat opengebroken apparaten niet-conform zijn.
- **Vereisen dat het apparaat zich op of onder het Mobile Threat Defense-niveau bevindt** (iOS 8.0 en hoger): Kies het maximale dreigingsniveau waarbij apparaten als niet-conform worden gemarkeerd. Apparaten die dit dreigingsniveau overschrijden, worden gemarkeerd als niet-conform:
  ...
  - **Laag**: het apparaat wordt als conform geëvalueerd als er alleen bedreigingen met een laag niveau op staan. Als een hoger niveau wordt aangetroffen, krijgt het apparaat de status niet-compatibel.
...
- **Wachtwoord vereist voor het ontgrendelen van mobiele apparaten**: **vereisen** dat gebruikers een wachtwoord invoeren voordat ze toegang kunnen krijgen tot hun apparaat. iOS-apparaten die gebruikmaken van een wachtwoord, zijn versleuteld.
...
- **Het aantal niet-alfanumerieke tekens in het wachtwoord**: Voer het aantal symbooltekens (zoals &, #, %, !, enzovoort) in dat het wachtwoord moet bevatten.
...
- **App-bundel-id**: voer de unieke bundel-id in die is toegewezen door de app-provider. Zie [De bundel-id vinden voor een iOS-app](https://support.microsoft.com/help/4294074/how-to-find-the-bundle-id-for-an-ios-app) om de bundel-id te vinden.  
