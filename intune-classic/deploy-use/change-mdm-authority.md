...
Nadat u overschakelt naar de nieuwe MDM-instantie, duurt het waarschijnlijk enige tijd (maximaal acht uur) voordat het apparaat wordt ingecheckt en synchroniseert met de service. U zult instellingen in de nieuwe MDM-instantie (hybride) moet configureren om ervoor te zorgen dat ingeschreven apparaten na de wijziging beheerd en beschermd blijven. Let op het volgende:
...
- Maak een Configuration Manager-gebruikersverzameling met alle gebruikers die momenteel worden beheerd door de zelfstandige versie van Intune. Die verzameling gebruikt u bij het instellen van het Intune-abonnement in de Configuration Manager-console. Hiermee zorgt u ervoor dat er een Configuration Manager-licentie is toegewezen aan de gebruikers en hun apparaten, en ze kunnen worden beheerd in de hybride omgeving na de wijziging van MDM-instantie.
...
- Voorafgaand aan de wijziging van MDM-instantie moet de MDM-instantie worden weergegeven als **Ingesteld op Microsoft Intune** (zelfstandige tenant) in de Microsoft Intune-beheerconsole.
    > [!NOTE]    
    > Als de MDM-instantie wordt weergegeven als **Beheerd door Intune en Office 365** worden uw door Office 365 beheerde MDM-apparaten niet langer beheerd wanneer u de MDM-instantie wijzigt naar **Configuration Manager** (hybride). We adviseren om die gebruikers te voorzien van een licentie voor Intune of Enterprise Mobility Suite voordat u de MDM-instantie wijzigt.   
...
4. Selecteer de gebruikersverzameling die alle gebruikers bevat die ook zullen worden beheerd door de nieuwe hybride MDM-instantie.
5. Klik op **Volgende** en voltooi de wizard. De MDM-instantie is nu gewijzigd naar **Configuration Manager**.
6. Meld u aan bij de [Microsoft Intune-beheerconsole](http://manage.microsoft.com) met dezelfde Intune-tenant om te controleren of de MDM-instantie is gewijzigd naar **Ingesteld op Configuration Manager**.
...
1. **Een aanvraag voor certificaatondertekening downloaden**
    ...
    > [!IMPORTANT]
    > U moet een nieuwe aanvraag voor certificaatondertekening downloaden. Als u een oud bestand gebruikt, mislukt de aanvraag.  
...
- Wanneer de Intune-service detecteert dat de MDM-instantie van een tenant is gewijzigd, wordt er een verzoek verzonden naar alle ingeschreven apparaten om in te checken en te synchroniseren met de service (dit staat los van het gebruikelijke geplande inchecken). Dus nadat de MDM-instantie voor de tenant is gewijzigd van de zelfstandige versie van Intune naar hybride, zullen alle ingeschakelde apparaten die online zijn verbinding maken met de service, de nieuwe MDM-instantie ontvangen en voortaan worden beheerd door hybride. Er vindt geen onderbreking plaats van het beheer en de beveiliging van deze apparaten.
...
- Er ontstaat een tijdelijke situatie als een apparaat offline is tijdens de wijziging van MDM-instantie en pas later incheckt bij de service. Om ervoor te zorgen dat het apparaat beveiligd is en blijft functioneren tijdens deze periode, blijft het volgende maximaal zeven dagen aanwezig op het apparaat (of tot wanneer het apparaat verbinding maakt met de nieuwe MDM-instantie en nieuwe instellingen ontvangt die de oude overschrijven):
    - E-mailprofiel
    - VPN-profiel
    - Certificaatprofiel
    - Wi-Fi-profiel
...
- Zorg dat de nieuwe instellingen, die de bestaande instellingen moeten overschrijven, dezelfde naam hebben als de vorige instellingen om er zeker van te zijn dat ze worden overschreven. Anders blijven er mogelijk achterhaalde profielen en beleidsregels achter op het apparaat.    
  ...
  > [!TIP]    
  > De beste methode is om alle beheerinstellingen en -configuraties en alle implementaties zo snel mogelijk te maken nadat de wijziging van MDM-instantie is voltooid. Dit zorgt ervoor dat apparaten beveiligd zijn en actief worden beheerd in de tijdelijke situatie.
